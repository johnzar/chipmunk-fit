import 'package:catalog/presentation/component/button/model/animated_bottom_button_catalog_models.dart';
...
class AnimatedBottomButtonControlPanel extends StatelessWidget {
  const AnimatedBottomButtonControlPanel({
    super.key,
    ...
    required this.onUseSafeAreaChanged,
    required this.labelController,
  });

  final FitButtonType selectedType;
  final ValueChanged<FitButtonType> onTypeChanged;
  final int maxLines;
  ...
  final ValueChanged<bool> onUseSafeAreaChanged;
    ...
    return _ControlCard(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildMaxLinesSelector(context, colors),
          const SizedBox(height: 16),
          _SwitchRow(
            title: 'Enabled',
            techKey: 'isEnabled',
          ...
          _SwitchRow(
            ...
            techKey: 'useSafeArea',
            value: useSafeArea,
            onChanged: onUseSafeAreaChanged,
          ),
          const SizedBox(height: 8),
          _buildLabelField(context, colors),
        ],
      ),
    );
  }
    ...
    return Column(
      ...
      children: [
        ...
        Wrap(
          spacing: 8,
          runSpacing: 8,
          children: animatedBottomButtonTypeItems.map((item) {
  ...
  Widget _buildMaxLinesSelector(BuildContext context, FitColors colors) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
        ...
        const SizedBox(height: 8),
        Row(
          children: animatedBottomButtonMaxLinesOptions.map((line) {
            final selected = line == maxLines;
            return Padding(
              padding: EdgeInsets.only(
                right: line == animatedBottomButtonMaxLinesOptions.last ? 0 : 8,
              ),
              child: FitChip(
                isSelected: selected,
  ...
  Widget _buildLabelField(BuildContext context, FitColors colors) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
